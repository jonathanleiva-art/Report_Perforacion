...
from schema import COALESCE_DUPLICATE_COLUMNS, COLUMNAS_CORREGIDAS, NUMERIC_COLUMNS, TEXT_TAG_COLUMNS
...
def fusionar_columnas_duplicadas(df):
    ...
    for columna in duplicadas:
        ...
        if columna in COALESCE_DUPLICATE_COLUMNS:
            resultado[columna] = bloque.apply(
                lambda fila: next(
                    (
                        valor_a_numero(valor)
                        for valor in fila
                        if valor_a_texto(valor)
                    ),
                    0,
                ),
                axis=1,
            )
        elif columna in NUMERIC_COLUMNS:
            resultado[columna] = bloque.apply(
