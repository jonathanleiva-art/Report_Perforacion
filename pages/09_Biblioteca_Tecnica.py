...
def _render_agregar_documento():
    app.st.subheader("Agregar documento técnico")
...
def _render_botones_equipo():
    app.st.subheader("Accesos rápidos por equipo")
...
def _render_listado(documentos):
    app.st.subheader("Documentos técnicos")
    columnas = [
        "titulo",
        "tipo_documento",
        "equipo_asociado",
    ...
    for _, fila in documentos.iterrows():
        ...
        with app.st.container(border=True):
            ...
            col1.markdown(f"**{texto_visible(documento.get('titulo'))}**")
            col1.caption(
...
def _render_sugerencias():
    app.st.subheader("Documentos sugeridos para cargar")
