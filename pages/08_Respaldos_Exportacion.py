...
def _mostrar_exportaciones(filtros):
    ...
    with col_datos:
        ...
        if app.st.button("Exportar ciclos de perforación", key="exportar_ciclos_perforacion"):
            df_ciclos = ciclos_service.leer_ciclos_operacional()
            app.st.session_state["export_ciclos_bytes"] = backup_service.dataframe_a_excel_bytes(df_ciclos)
            app.st.session_state["export_ciclos_count"] = len(df_ciclos)
        if "export_ciclos_bytes" in app.st.session_state:
            app.st.download_button(
                "Descargar ciclos de perforación",
                data=app.st.session_state["export_ciclos_bytes"],
                file_name="ciclos_perforacion_export.xlsx",
                mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
            )
            app.st.caption(f"Ciclos exportados: {app.st.session_state.get('export_ciclos_count', 0)}")
...
def main():
    if not app.requerir_acceso(admin=True):
        return
    render_page_header(app.st, "Respaldos y Exportación")
    app.st.caption(
